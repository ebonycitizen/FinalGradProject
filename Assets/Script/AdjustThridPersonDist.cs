...
public class AdjustThridPersonDist : MonoBehaviour
{
    ...
    {
        Gizmos.DrawWireCube(thirdPersonSize.bounds.center, thirdPersonSize.bounds.size);

        if (isHit)
        {
            Gizmos.DrawRay(originAxis.position, originAxis.forward * hit.distance);
            Gizmos.DrawWireCube(originAxis.position + originAxis.forward * hit.distance, thirdPersonSize.bounds.size);
            //Debug.Log(originAxis.transform.InverseTransformPoint(hit.point));
        }
        else
            Gizmos.DrawRay(originAxis.position, originAxis.forward * rayDistance);
    }
}
